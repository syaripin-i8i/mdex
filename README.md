...
`enrich` の更新は `node_overrides` に即時反映されるため、`find` / `first` / `related` / `context` で再利用するだけなら再 `scan` は不要です。Markdown 本文を更新したときだけ `scan` で seed index を再構築してください。
...
書く            規約に沿って Markdown を残す  →  docs/convention.md
  ↓
seed 更新       mdex scan（Markdown から索引を再構築）
  ↓
読む            mdex find / first / related / query（必要なら context）
  ↓
